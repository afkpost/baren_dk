...
var App, CacheStore, ServerConnection, push;

push = $("<Push.bus>");

window.androidPush = function(e) {
  var data;
  if (e.event === "registered") {
    push.trigger("gcm_regid", e.regid);
  } else if (e.event === "message") {
    data = e.payload;
    if (e.foreground) {
      if (data.update != null) {
        console.log("triggering update " + data.update);
        push.trigger("update", data.update);
      }
    }
  } else {
...
App = (function() {
  function App(conn, log) {
    var bus, get, parseDate, platform, pushError, pushNotification, pushSuccess, put, store, tokenHandler, updateDrinks, updateEvents, updateNews;
    if (log == null) {
      log = function() {};
    }
    bus = $('<App.bus>');
    ...
    this.updateRanks();
    push.on("gcm_regid", (function(_this) {
      return function(e, regid) {
        return conn.updateUser({
          regid: regid
        });
      };
    })(this));
    push.on("update", (function(_this) {
